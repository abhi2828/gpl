...
import { getModuleDropdown } from '../modules/store'
...
const BulkUploadModal = ({ setShowBulkUploadModal, showBulkUploadModal }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const module = moduleDropdown.filter(
      (temp) => temp.moduleName === 'LearningTrack'
    )

    if (module.length === 0) {
      toast.error(
        'Learning Tracks module not present. Please add module in module master.'
      )
      return
    }
      ...
      >
        ...
        <ModalBody>
          <div className="text-center mb-0">
            <h1 className="mb-0">BulkUpload</h1>
          </div>
          ...
          <Row tag={Form} className="p-2" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Label className="form-label" for="file">
              {/* <a href='../../../public/csv/LearningTrackCsvFile.xlsx' download>Click Here   :  </a>  */}
              <Link to="/csv/Learning_track.csv" target="_blank" download>
                {' '}
                Click Here{' '}
              </Link>
