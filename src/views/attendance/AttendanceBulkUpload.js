...
import { attendanceBulkUpload, setLoader } from './store'
...
import { getModuleDropdown } from './../masters/modules/store/index'
...
const AttendanceBulkUpload = ({
  ...
  const onSubmit = async (data) => {
    ...
    const module = moduleDropdown.filter(
      (temp) => temp.moduleName === 'Attendance'
    )

    if (module.length === 0) {
      toast.error(
        'Attendance module not present. Please add module in module master.'
      )
      return
    }
      ...
      >
        ...
        <ModalBody>
          <div className="text-center mb-0">
            <h1 className="mb-0">Attendance Bulk Upload</h1>
          </div>
          ...
          <Row tag={Form} className="p-2" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Label className="form-label" for="file">
              <Link to="/csv/attendanceSample.csv" target="_blank" download>
                {' '}
                Click Here{' '}
              </Link>
